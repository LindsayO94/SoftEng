...
public class SelectMove extends gameMove{
	...
	public boolean doMove(Game game) {
		if(valid(game)==true)
		{
			...
			{
			System.out.println("inside empty array");
			...
			case TILE_CELL:
				...
				return true;
			case INACTIVE_CELL:
				throw new IllegalArgumentException("You may not select an inactive cell");
			case BASKET_CELL:
				throw new IllegalArgumentException("You may not select a basket cell");
			}
			...
			else
			{
				...
				case TILE_CELL:
					...
					{
						System.out.println("Inside non empty array");
						((TileCell) cell).setSelected();
						return true;
					}
					else
